...
- **Secure temp handling** — when building a ZIP, tracks are staged in a
  ...
  folder next to your chosen ZIP location, and that temp directory is now
  always cleaned up afterward, including if the download run fails
  partway through.
- **Validated proxy input** — the Proxy URL field is checked when you save
  it, so a typo or malformed address is caught immediately with a clear
  message instead of failing confusingly later.
- **System proxy variables ignored by default** — if your OS has stray
  `http_proxy`/`https_proxy` environment variables set (e.g. left over
  from a VPN tool), the app won't silently pick them up unless you've
  actually set a proxy in its own Settings.
- **Download integrity check** — if a track's connection drops mid-download
  and produces a truncated file, it's now detected and reported as failed
  (so you can retry it) instead of being silently kept as a broken file.
- **Optional debug log** — a toggle in Settings writes a `debug.log` with
  full error details for troubleshooting, kept locally with owner-only
  permissions and off by default. "Clear all saved data" removes it too.
