...
class SoundCloudGrabberApp(tb.Window):
    def __init__(self):
        ...
        settings_module.set_debug_logging(self.settings.get("debug_log_enabled", False))
    ...
    def _build_settings_tab(self):
        ...
        self.debug_log_var = tb.BooleanVar(value=self.settings.get("debug_log_enabled"))
        tb.Checkbutton(
            frame, text="Write a debug.log for troubleshooting (kept on your device only)",
    ...
    def _save_settings_clicked(self):
        proxy_error = settings_module.validate_proxy_url(self.proxy_var.get())
        if proxy_error:
            messagebox.showerror("Invalid proxy URL", proxy_error)
            return
        ...
        self.settings["debug_log_enabled"] = self.debug_log_var.get()
        settings_module.save_settings(self.settings)
        settings_module.set_debug_logging(self.settings["debug_log_enabled"])
    ...
    def _on_clear_data(self):
        confirmed = messagebox.askyesno(
            ...
            "This deletes your saved settings file, recent links, cached client id, download "
            "folder preference, and debug log (if any). This cannot be undone. Continue?",
        )
        if not confirmed:
            return
        ...
        settings_module.set_debug_logging(False)
    ...
    def _test_connection_worker(self):
        try:
            ...
            settings_module.log_error("test_connection failed", exc)
    ...
    def _fetch_worker(self, url):
        try:
            ...
            settings_module.log_error(f"resolve failed for url={url!r}", exc)
    ...
    def _search_worker(self, query):
        try:
            ...
            settings_module.log_error(f"search failed for query={query!r}", exc)
    ...
    def _download_worker(self, items, folder, zip_path):
        ...
        try:
            self._download_worker_inner(items, folder, zip_path, temp_folder)
        finally:
            if cleanup_temp:
    ...
    def _download_worker_inner(self, items, folder, zip_path, temp_folder):
        try:
            ...
            settings_module.log_error("download_batch failed", exc)
        ...
        if zip_path and batch.downloaded:
            try:
                ...
                settings_module.log_error("building ZIP failed", exc)
